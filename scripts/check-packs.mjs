...
for (const entry of entries) {
  ...
  //
  // --force is load-bearing. `tsc --build` is incremental against
  // tsconfig.tsbuildinfo, and that file is not gitignored alongside dist/. A
  // tree holding build info whose dist/ has since gone is therefore judged up
  // to date, emits nothing, and packs exactly the src-only tarball this hook
  // exists to prevent — reachable from a failed build, an interrupted one, or
  // anything that clears dist/ without clearing the cache beside it.
