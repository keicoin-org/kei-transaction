...
> **Status: M0.** The API is real and runs end to end; the chain underneath it is
> an in-memory mock. There is no network, no testnet, and nothing here holds
> value. See [Where this is](#where-this-is).
...
M0 of eleven. What exists:

| | |
|---|---|
| **The §6.7 API** | Complete, running end to end, types published |
| **The chain** | An in-memory mock enforcing the SPEC §5.6 / §7 ledger rules |
| **The network** | None. `Kei.start()` with no node gets a private in-process chain, and `kei.network` reports `'mock'` |
| **The market** | M5 — `@kei/market` does not exist yet |
...
|---|---|
| `@kei/core` | wallet, send, receive, blocks, node clients, the mock ledger |
| `@kei/tokens` | issue, mint, burn, transfer, policy flags, items |
| `@kei/claims` | Merkle roots, proofs, claim blocks |
| `@kei/work` | proof-of-work tiers, local generation, work-server client |
| `@kei/wallet` | in-game wallet: balances, inventory, pending claims |
| `@kei/market` | M5 |

`@kei/core` depends on nothing else in the tree.
