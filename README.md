...
| **npm** | The initial `0.8.0` graph published and verified on 4 August 2026 was core `0.5.0`, work `0.4.1`, claims `0.5.1`, tokens `0.5.2`, market `0.4.0`, wallet `0.5.0`, economy `0.2.2`, player economy `0.1.2`, and umbrella `0.8.0`. It is no longer the whole of what the registry serves: **a release run on 5 August 2026 published six packages on top of it and then stopped**, so npm now also holds core `0.6.0`, work `0.4.2`, claims `0.6.0`, tokens `0.5.3`, market `0.5.0` and wallet `0.5.1`, with no umbrella pinning any of them. `npm install kei-transaction` still installs `0.8.0` and its public entry still imports, but a fresh install measured on 5 August 2026 resolves **four copies of `@keicoin/core`**: `0.5.0` hoisted, and `0.6.0` nested separately under `tokens`, `wallet` and `work`. The umbrella's `^0.5.2` / `^0.5.0` / `^0.4.1` floors *can* reach the newly published `tokens 0.5.3`, `wallet 0.5.1` and `work 0.4.2`, and each of those requires `core ^0.6.0`; `claims`, `market`, `economy` and `player economy` cannot reach across their own minors, so they stay against `core 0.5.0`. Core holds the client and the shared asset cache, so a `0.8.0` install today runs two different cores side by side in a combination nothing has tested. `@keicoin/core@0.6.0` is burnt: the tarball on npm predates [#141](https://github.com/keicoin-org/kei-transaction/pull/141) and lacks three symbols the umbrella re-exports, so it can never be the core this tree ships against. [#157](https://github.com/keicoin-org/kei-transaction/issues/157) is the diagnosis and the `0.9.0` candidate below is the fix. `create-kei-game@0.2.0` predates the harness's move to its standalone repository; future harness releases are owned there |
...
One copy of core per install is the acceptance criterion, and the **npm** row
above is what the registry fails it with today: four copies, `0.5.0` hoisted and
`0.6.0` nested three times. Packing all nine tarballs at the versions in the
table and installing them together gives **one** — the check `publish.sh` runs as
`smoke-pack-install.mjs`, which also imports every public entry and is therefore
the check that fails outright for the graph as it was numbered before core moved
to `0.7.0`. That proves the graph is internally consistent and importable. It
does *not* perform range resolution against the registry, so whether a real
`npm install kei-transaction@0.9.0` dedupes to a single core is a thing to
measure after the publish, not before it.
