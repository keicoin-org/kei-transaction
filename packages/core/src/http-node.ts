...
/**
 * The node URL as it can safely appear in an error message.
 *
 * A node URL is somewhere people keep credentials — `https://user:pass@node/`,
 * an API key in the query string, a project token as a path segment — and error
 * messages get pasted into issues, logs and chat. What survives is the part
 * that answers "which node went quiet": scheme, host, path. Userinfo, query and
 * fragment do not, because none of them are needed to answer that.
 */
...
export class HttpNode implements KeiNode {
  ...
  constructor(options: HttpNodeOptions) {
    ...
    this.endpoint = sanitizeEndpoint(options.url)
